            ...
            <div class="p-6">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="{{ route('categories.index') }}"
                        class="block px-4 py-2 rounded {{ str_starts_with($routeName, 'categories') ? 'bg-gray-700' : 'hover:bg-gray-800' }}">
                        📂 Kategoriler
                    </a>
                    <a href="{{ route('tags.index') }}"
                        class="block px-4 py-2 rounded {{ str_starts_with($routeName, 'tags') ? 'bg-gray-700' : 'hover:bg-gray-800' }}">
                        🏷️ Etiketler
                    </a>
                    <a href="{{ route('posts.index') }}"
                        class="block px-4 py-2 rounded {{ str_starts_with($routeName, 'posts') ? 'bg-gray-700' : 'hover:bg-gray-800' }}">
                        📝 Yazılar
                    </a><hr>
...
<header class="bg-gray-900 text-white flex items-center justify-between px-6 py-4">
    ...
    <div class="relative" x-data="{ open: false }">
        ...
        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white text-gray-700 rounded-md shadow-lg z-50">
