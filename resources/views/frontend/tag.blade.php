...
<head>
    ...
    <title>{{ $tagTranslation->seo_title }} - Multiblog</title>
    <meta name="description" content="{{ $tagTranslation->seo_description }}">
    <meta name="keywords" content="{{ $tagTranslation->seo_keywords }}">
...
<body class="bg-gray-50 antialiased">
                ...
                <div class="relative group" x-data="{ open: false }" x-init="open = false">
                    ...
                    <div x-show="open"
                        ...
                        {{-- @foreach(config('app.languages') as $language)
                            <a href="{{ route('tag.show', [$language, $tag->translations->firstWhere('language_slug', $language)->slug]) }}"
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm p-6 lg:p-8">
            <div class="mb-8">
                <h1 class="text-3xl font-bold mb-4">{{ $tagTranslation->name }}</h1>
                <p class="text-gray-600">{{ $posts->total() }} posts in this tag</p>
            </div>
