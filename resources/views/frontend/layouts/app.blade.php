            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="relative group" x-data="{ open: false }" x-init="open = false">
                    <button @click="open = !open"
                        ...
                        <img src="https://flagcdn.com/{{ $activeLang->flag }}.svg" class="w-6 h-4 rounded-sm shadow"
                            alt="{{ $lang }}">
                    ...
                    <div x-show="open"
                        ...
                        @foreach ($languages as $l)
                            @php
                                $targetSlug = $translatedSlugs[$l->slug] ?? ($slug ?? null);
                                $targetUrl = isset($targetSlug)
                                    ? route(Route::currentRouteName(), ['lang' => $l->slug, 'slug' => $targetSlug])
                                    : route('frontend.home', ['lang' => $l->slug]);
                            @endphp
                            <a href="{{ $targetUrl }}"
